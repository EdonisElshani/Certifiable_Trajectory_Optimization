...
For $g\in G$ and $\eta\in\mathfrak{g}=T_eG$, a local variation of
$g$ is defined by
\begin{equation}
    g^\epsilon = g\exp(\epsilon\eta),
\end{equation}
where $\epsilon\in (-c,c)$ is the variation parameter for $c>0$. Since the
variation is constructed through the group operation, $g^\epsilon$
remains in $G$ \cite{Lee2008}. 

\begin{proposition}[Infinitesimal variation on a matrix Lie group]
    Let $G$ be a matrix Lie group with Lie algebra
    $\mathfrak{g}=T_eG$ \cite{Hall2000LieGroups}. For any
    $g\in G$ and $\eta\in\mathfrak{g}$, define the smooth curve
    $\gamma:(-c,c)\rightarrow G$ by
    \begin{equation}
        \gamma(\epsilon)
        =
        g\exp(\epsilon\eta),
...
\begin{proof}
For any $\eta\in\mathfrak{g}$, the curve
$\gamma(\epsilon)=g\exp(\epsilon\eta)$ is a smooth curve on $G$
satisfying $\gamma(0)=g$. Therefore,
\begin{align}
\delta g
&=
\dot{\gamma}(0)
...
T_eL_g(\eta),
\end{align}
where $\delta g$ is the infinitesimal variation of $g$ induced by
$\eta$ \cite{Lee2008}.
\end{proof}
