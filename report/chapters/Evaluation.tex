...
\end{align}
 which is projected on $SO(3)$. Consequently, $\bm{R}_{n+1}$ satisfy the orthogonality constraint, while the angular velocity $\bm{\omega}_{n+1}$ remains unchanged on the vector space $\mathbb{R}^{3 \times 3}$. However, this method does not inherit structure preserving properties of the Lie group variational integrator, for example, symplecticity or momentum conservation \cite{CELLEDONI2003421}. 
...
\begin{figure}[t]
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \caption*{(a) Energy deviation $\Delta E_k$.}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
