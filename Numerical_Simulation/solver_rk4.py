...
def rk4_step(model, R: np.ndarray, Omega: np.ndarray, h: float) -> tuple[np.ndarray, np.ndarray]:
    """
    Explicit RK4 step for the state y = (R, Omega).
    """
    k1_R, k1_Om = model.rk4_rhs(R, Omega)
    k2_R, k2_Om = model.rk4_rhs(R + 0.5 * h * k1_R, Omega + 0.5 * h * k1_Om)
    k3_R, k3_Om = model.rk4_rhs(R + 0.5 * h * k2_R, Omega + 0.5 * h * k2_Om)
    k4_R, k4_Om = model.rk4_rhs(R + h * k3_R, Omega + h * k3_Om)
...
def simulate_rk4(model, R0: np.ndarray, Omega0: np.ndarray, h: float, tf: float,
    ...
    for k in range(N):
        R, Omega = rk4_step(model, R, Omega, h)
